import { formatDateKey } from "./formatter.js";
...
<body>
  <main class="shell homeShell">
    ...
    <section class="homeTopBand" aria-label="AI HOT 情报工作台概览">
  ...
  <main class="shell">
    ...
    <header class="featureHero libraryFeatureHero" aria-label="知识库主视觉">
      <div class="featureHeroCopy">
        ...
        <div class="heroActionBar">
          <a class="primaryLink" href="#libraryFinder">回到筛选</a>
...
function renderReviewPage(archives, url) {
  ...
  const latestDate = safeArchives[0]?.date || "";
  const reviewHeadline = reviewHeroHeadline(review, days);
  const coverageText = days === review.daysCovered
    ? `当前实际覆盖最近 ${review.daysCovered} 天`
    : `你选择的是 ${days} 天窗口，当前实际覆盖最近 ${review.daysCovered} 天`;
  ...
  <main class="shell reviewShell">
    ...
    <header class="featureHero reviewFeatureHero" aria-label="阶段复盘主视觉">
      <div class="featureHeroCopy">
        ...
        <p class="muted">${escapeHtml(coverageText)}、${review.cardCount} 张卡片；这里看高频方向、论文线索、工具项目和来源变化。${escapeHtml(review.summary)}</p>
        ...
        <div class="heroMicroStats heroCapsules" aria-label="复盘状态">
          <span><b>${review.daysCovered}</b> 天归档</span>
          <span><b>${review.cardCount}</b> 张卡片</span>
          <span><b>${review.paperCount}</b> 条论文</span>
        </div>
      </div>
      ${renderReviewHeroScene(review, selectedArchives, days)}
    </header>
    ...
    <section class="pageAtlas reviewAtlas" aria-label="周期趋势">
      <div class="atlasLead">
        <span>周期趋势</span>
        <h2>归档量、类型和来源</h2>
        <p>${escapeHtml(coverageText)}、共 ${review.cardCount} 张卡片。趋势、类型和来源用来决定下一轮先读哪一类内容。</p>
      </div>
    ...
    <section class="reviewPanel reviewOverview" aria-label="阶段概览">
      <div class="reviewHead">
        <div>
          ...
          <h2>${review.daysCovered} 天归档里有什么变化</h2>
...
function reviewHeroHeadline(review, days) {
  const covered = Number(review?.daysCovered) || 0;
  if (covered && covered !== days) return `最近 ${covered} 天复盘（${days} 天窗口）`;
  return `${days} 天复盘`;
}
...
function renderReviewHeroScene(review, archives, days) {
  return `<div class="featureScene reviewScene" aria-hidden="true">
    <div class="sceneHeadline">
      <span>${days} 天窗口</span>
      <strong>${review.daysCovered} 天 / ${review.cardCount} 张</strong>
    </div>
...
function renderHomeTodayBoard({ date, latestHref, latestArchive, cards, latestSummary, paperCount, toolCount, sourceCount, typeSummary, sourceRanks }) {
  ...
  return `<section class="homeTodayBoard" aria-label="今日判断">
    <div class="todayBoardHeader">
      ...
      <div class="todayBoardActions">
        <a class="primaryLink" href="/library">进入知识库</a>
        <a class="navButton" href="/review?days=30">看阶段复盘</a>
        <a class="navButton" href="${latestHref}">读最新日报</a>
      </div>
    </div>
...
function renderHomeControlCenter({ archives, cards, typeSummary, sourceRanks, review }) {
  ...
  const peakArchive = safeArchives.reduce((peak, archive) => {
    const count = Array.isArray(archive.cards) ? archive.cards.length : 0;
    const peakCount = Array.isArray(peak?.cards) ? peak.cards.length : 0;
    return count > peakCount ? archive : peak;
  }, safeArchives[0] || null);
  const peakCount = Array.isArray(peakArchive?.cards) ? peakArchive.cards.length : 0;
  const typeInsight = types[0]
    ? `${types[0].name}最多，先从这个方向筛选。`
    : "等待更多归档后再判断方向。";
  const sourceInsight = sources[0]
    ? `${sources[0].name}贡献最多，适合检查连续更新。`
    : "等待来源稳定后再看集中度。";
  return `<section class="homeControlCenter" aria-label="近 30 天运行概览">
    ...
    <article class="controlPanel cadencePanel">
      <div class="controlPanelHead">
        <span>最近 14 天</span>
        <h3>归档节奏</h3>
        <p>${peakArchive ? `${escapeHtml(peakArchive.date)} 最高，${peakCount} 张卡片；` : ""}点击柱子进入当天归档。</p>
      </div>
...
function renderMobileLibraryBrief({ rangeText, latestDate, cards, categories, sourceRanks, paperCount, toolCount }) {
  return `<section class="mobileOnly mobilePageBrief mobileLibraryBrief" aria-label="手机端知识库导览">
    <div class="mobileSectionHead">
      ...
      <p>${escapeHtml(rangeText)}，${cards.length} 张卡片。输入关键词，再按类型和日期缩小范围，先把结果压到可读数量。</p>
    </div>
...
function renderMobileReviewBrief({ review, archives, categories, sources, days }) {
  const coverageText = days === review.daysCovered
    ? `当前覆盖 ${review.daysCovered} 天`
    : `${days} 天窗口，当前覆盖 ${review.daysCovered} 天`;
  const title = days === review.daysCovered ? "阶段复盘" : `最近 ${review.daysCovered} 天复盘`;
  return `<section class="mobileOnly mobilePageBrief mobileReviewBrief" aria-label="手机端阶段复盘摘要">
    <div class="mobileSectionHead">
      <span>${days} 天窗口</span>
      <strong>${escapeHtml(title)}</strong>
      <p>${escapeHtml(compactPageText(`${coverageText}，${review.summary}`, 104))}</p>
    </div>
    ...
    <section class="mobileDataBoard">
      <article class="mobileDataCard">
        <div class="mobileMiniHead"><span>归档走势</span><b>${review.daysCovered} 天</b></div>
    ...
    ${renderMobileSourceRows(sources, "主要来源")}
  </section>`;
}
...
function renderMobileSourceRows(items, title) {
  ...
  return `<section class="mobileSourceCard" aria-label="${escapeAttribute(title)}">
    ...
    <div class="mobileSourceRows">
      ${list.map((item) => `<p><span>${escapeHtml(compactPageText(item.name, 20))}</span><b>${Number(item.count) || 0}</b></p>`).join("")}
    </div>
  </section>`;
}
  ...
  <main class="shell">
    ...
    <section class="dailyReadOrder" aria-label="推荐阅读顺序">
      <div>
        <span>优先阅读</span>
        <h2>优先读这三条</h2>
        <p>先看分数最高的三条，确认事实、用途和原文，再继续扫全量卡片。</p>
      </div>
...
function dailyPageCSS() {
  return `
    ...
    * { box-sizing: border-box; }
    ...
    .todayBoardActions .primaryLink,
    .todayBoardActions .navButton {
      min-height: 42px;
      padding: 0 14px;
      box-shadow: none;
    }
    ...
    /* Unified surface pass: hero bands own their backgrounds, cards stay quiet. */
    .homeTopBand {
      position: relative;
      ...
      border: 1px solid color-mix(in oklch, var(--line) 78%, white);
      border-radius: 20px;
      background: linear-gradient(112deg, oklch(0.948 0.02 182), oklch(0.985 0.005 214) 56%, oklch(0.955 0.018 224));
      overflow: hidden;
    }
    .homeTopBand .homeArtHero {
      margin: 0;
    ...
    .featureHero {
      ...
      border: 1px solid color-mix(in oklch, var(--line) 78%, white);
      border-radius: 20px;
      background: linear-gradient(112deg, oklch(0.95 0.018 184), oklch(0.985 0.005 216) 58%, oklch(0.955 0.017 226));
      overflow: hidden;
    }
    :is(.siteNav, .finderPanel, .card, .controlIntro, .controlPanel, .homeHeroBrief, .featureScene, .reviewPanel, .reviewGlassBlock, .reviewTimeline, .reviewSamples, .dailyReadOrder, .pageAtlas, .routePane, .todayLead, .todayQueueCard, .todayFacts, .mobileTodayCard, .mobilePageBrief) {
      backdrop-filter: none !important;
      -webkit-backdrop-filter: none !important;
      box-shadow: 0 1px 2px color-mix(in oklch, oklch(0.3 0.03 230) 8%, transparent) !important;
    }
    .featureScene {
      transform: none !important;
      background: color-mix(in oklch, white 72%, transparent) !important;
    }
    .libraryFeatureHero {
      min-height: auto !important;
      grid-template-columns: minmax(0, 0.86fr) minmax(260px, 0.44fr) !important;
      align-items: stretch;
      gap: clamp(18px, 2.4vw, 28px) !important;
      padding: clamp(18px, 2.4vw, 24px) !important;
    }
    .libraryFeatureHero .featureHeroCopy {
      align-content: center;
    }
    .libraryFeatureHero h1 {
    ...
    .libraryFeatureHero .muted {
      max-width: 66ch;
      font-size: 15.5px !important;
      line-height: 1.62 !important;
    }
    .libraryFeatureHero .featureScene {
      min-height: 210px !important;
      max-height: 240px;
      overflow: hidden;
    }
    .finderTop {
      grid-template-columns: minmax(240px, 1fr) 170px minmax(0, 1.1fr) !important;
    }
    .advancedFilterGrid {
      grid-template-columns: repeat(4, minmax(0, 1fr));
    }
    ...
    @media (max-width: 760px) {
      ...
      body {
        min-width: 0;
        background: linear-gradient(180deg, oklch(0.988 0.004 210), oklch(0.975 0.01 190));
        font-size: 16px;
      }
      body::before {
        opacity: 0.16;
      ...
      .siteNav {
        ...
        background: color-mix(in oklch, white 92%, transparent) !important;
        overflow: visible !important;
      }
      .brandMark,
      .navStatus {
      ...
      .navLinks a.active {
        ...
        background: color-mix(in oklch, var(--accent-soft) 86%, white) !important;
      }
      .homeTopBand {
        margin: 0;
        padding: 0;
      ...
      .mobileTypeRows i {
        ...
        height: 100%;
        border-radius: inherit;
        background: color-mix(in oklch, var(--accent) 78%, white);
      }
      .mobileSignalStack,
      .mobileHomeEntrances,
      .readOrderList,
      ...
      .search input {
        ...
        background: color-mix(in oklch, white 90%, transparent) !important;
      }
      #clearFilters {
